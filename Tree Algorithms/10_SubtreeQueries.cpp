...
vi treeTraversalArray;
ll segmentTree[2000000];
void dfs1(int s,int p){
    indexNodes[s]=treeTraversalArray.size();
    treeTraversalArray.push_back(values[s]);
    subTreeSize[s]=1;
    for(int u:adj[s]){
        if(u==p) continue;
        dfs1(u,s);
...
void build(int r,int l=0,int si=0){
    if(r==l) {
        segmentTree[si] = (ll)treeTraversalArray[l];
        return;
    }
    int mid = l + (r-l)/2;
    build(mid,l,2*si+1);
...
void solving(int& n){
    ...
    if(a==1){
        cin>>c;
        update(indexNodes[b],c,n);
    }else{
        int k = indexNodes[b]+subTreeSize[b]-1;
        cout<<getSum(indexNodes[b],k,n)<<'\n';
    }
}
int main(){
